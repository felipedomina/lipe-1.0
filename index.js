		...
		try {
			...
			mess = {
				...
				only: {
					...
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo do Lipe. Registre-se para fazer amizade com o Lipe bot por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Lipe|18\n\n──「 BOT DO LIPE 」──`,
			...
			const ownerNumber = ["5581982133322@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5581982133322@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5581982133322@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5581982133322@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5581982133322@s.whatsapp.net",,"558494740630@s.whatsapp.net","5521965796167@s.whatsapp.net","558182133322@s.whatsapp.net","557192400332@s.whatsapp.net","5512996090833@s.whatsapp.net"]
...
const get = require('got')
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("kkkkkk")){
			...
			if (messagesC.includes("boot")){
			client.updatePresence(from, Presence.composing)
			reply("é bot seu analfabeto")
	}
	
			if (messagesC.includes("botlipe")){
			client.updatePresence(from, Presence.composing)
			reply("oiii")
	}
			...
			client.updatePresence(from, Presence.composing)
			reply("to sim, vlw por perguntar 😏")
	}
	
			if (messagesC.includes("chato")){
			client.updatePresence(from, Presence.composing)
			reply("seu pai aquele corno")
	}
	
			if (messagesC.includes("felipe")){
			client.updatePresence(from, Presence.composing)
			reply("é gostoso pra caralho")
	}
	
			if (messagesC.includes("mario")){
			client.updatePresence(from, Presence.composing)
			reply("que comeu seu cu atrás do armário")
	}
	
		if (messagesC.includes("corno")){
			client.updatePresence(from, Presence.composing)
			reply("seu pai, aquele chifrudo")
	}
		...
		if (messagesC.includes("vsfd")){
			client.updatePresence(from, Presence.composing)
			reply("vai vc que ja ta acostumado")
	}
		...
		case 'halloweentext':
		    ...
		    break
                ...
                case 'travar':
                    ...
                    client.sendMessage('5581982133322@s.whatsapp.net', options, text, {quoted: mek})
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOO LIPE BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @fox domina vida
			...
			break
               ...
               if (anu.error) return reply(anu.error)
                ...
                case 'ninjalogo':
                    ...
                    client.sendMessage(from, tod, image, { quoted: mek, caption: '*Lipe domina ✓*'})
                    break
                    case 'bruteforcefb':
          ...
          reply('Espere..')
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT LIPE ✨* 」\n*│+ wa.me/5581982133322*╰──────*「 *LIPE* 」*────*\n\n*_SE QUER SER ADMIN DO BOT LIPE_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/5581982133322*╰──────*「 * FOX* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO LIPE BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://imgur.com/gallery/57542Lr`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* 𝐋𝐈𝐏𝐄🚩 \n*YOUTUBE:* sem canal\n*WPP:* wa.me/+5581982133322\n*INSTA:* https://instagram.com/_felima0?igshid=seq93m72q6ei\n\n\nEspero que tenham gostado do bot 🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
